...
### Step 1: PWA 웹앱 (즉시)
- URL 하나로 바로 사용 가능 (Vercel/Netlify 등 정적 호스팅)
- OPFS 기반 가상 파일 시스템 + 가져오기/내보내기
- 개발 속도 빠르고, 수정사항 즉시 반영

### Step 2: Capacitor 네이티브 래핑 (필요시)
- 웹 코드 90% 재사용, Capacitor로 iOS 앱 래핑
- `@capacitor/filesystem`으로 실제 파일 시스템 접근 확보
- TestFlight로 배포 (App Store 심사 없이 테스트 가능)
- Xcode 빌드에 Mac 필요

### 왜 이 순서인가?
- iPad Safari에서 Chromium 엔진 사용 불가 (Apple 정책, EU 제외)
- PWA로 빠르게 검증 후, 파일 접근 제약이 불편하면 Capacitor로 전환
- 코드를 버리지 않고 점진적으로 네이티브 기능 추가 가능
